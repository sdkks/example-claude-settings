#!/bin/bash
# auto-approve-compound-go.sh — PermissionRequest hook
# Trigger: PermissionRequest
# Matcher: Bash
#
# Approves compound commands composed entirely of Go toolchain ops.
# Covers: go, gofmt, goimports, golangci-lint, cd, and output helpers.

INPUT=$(cat)
COMMAND=$(echo "$INPUT" | jq -r '.tool_input.command // empty' 2>/dev/null)
[ -z "$COMMAND" ] && exit 0

# Must contain go toolchain to be relevant
echo "$COMMAND" | grep -qE '\b(go|gofmt|goimports|golangci-lint)\b' || exit 0

HOOK_NAME="auto-approve-compound-go"
...
# Reject immediately if heredoc or rm present anywhere
echo "$COMMAND" | grep -qE '<<|[[:space:]]rm[[:space:]]|[[:space:]]rm$|\brm -' && exit 0

# Strip common redirections before splitting so "2>&1" doesn't produce a bare "1" token
CLEANED=$(echo "$COMMAND" | sed 's/2>&[0-9]//g; s/>[[:space:]]*/>/g; s/2>\/dev\/null//g; s/>[^&]*\/dev\/null//g')
...
while IFS= read -r part; do
  ...
  # Strip leading KEY=val env assignments (e.g. GONOSUMCHECK=* go test ./...)
  part=$(echo "$part" | sed 's/^[[:space:]]*\([A-Z_][A-Z_0-9]*=[^[:space:]]*[[:space:]]\)*//')
  [ -z "$part" ] && continue

  # Allow: cd, go toolchain, output helpers
  if echo "$part" | grep -qE '^(cd |go |gofmt |goimports |golangci-lint |echo |ls |head |tail |grep |cat |wc |true$|false$|[0-9]+$)'; then
    continue
  fi
  # Allow bare commands with no args
  if echo "$part" | grep -qE '^(go|gofmt|goimports|golangci-lint|true|false)$'; then
    continue
  fi
