#!/bin/bash
# auto-approve-compound-npm.sh — PermissionRequest hook
# Trigger: PermissionRequest
# Matcher: Bash
#
# Approves compound commands composed of npm/npx/node ops.
# Excludes: npm publish, npm unpublish, npm adduser, heredocs.

INPUT=$(cat)
COMMAND=$(echo "$INPUT" | jq -r '.tool_input.command // empty' 2>/dev/null)
[ -z "$COMMAND" ] && exit 0

HOOK_NAME="auto-approve-compound-npm"

# Must contain npm/npx/node to be relevant
echo "$COMMAND" | grep -qE '\b(npm|npx|node)\b' || exit 0
...
# Reject if heredoc, rm, or publish present anywhere
echo "$COMMAND" | grep -qE '<<|[[:space:]]rm[[:space:]]|[[:space:]]rm$|\brm -' && exit 0
echo "$COMMAND" | grep -qE '\bnpm (publish|unpublish|adduser|owner|access|deprecate)\b' && exit 0
...
while IFS= read -r part; do
  ...
  # Strip leading KEY=val env assignments (e.g. NODE_ENV=test npm run test)
  part=$(echo "$part" | sed 's/^[[:space:]]*\([A-Z_][A-Z_0-9]*=[^[:space:]]*[[:space:]]\)*//')
  [ -z "$part" ] && continue

  # Allow: cd, npm/npx/node, fnm, output helpers
  if echo "$part" | grep -qE '^(cd |npm |npx |node |fnm )'; then continue; fi
  if echo "$part" | grep -qE '^(npm|npx|node|fnm)$'; then continue; fi
